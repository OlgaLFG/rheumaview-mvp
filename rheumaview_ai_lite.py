...
if "SI Joints" in regions:
    ...
    si_level = st.radio("Confidence level (SI):", ["Low", "Moderate", "High"])
    if si_level == "Low":
        st.text_area("AI-style phrasing:", 
            ...
            "Findings may represent early sacroiliitis or degenerative change.", height=150)
    elif si_level == "Moderate":
        st.text_area("AI-style phrasing:", 
            "Mild irregularity and sclerosis of bilateral sacroiliac joints, more pronounced on iliac sides. "
            "No erosions or ankylosis. Findings suggest early sacroiliitis.", height=150)
    else:
        st.text_area("AI-style phrasing:", 
            "Bilateral sacroiliac joint sclerosis, erosions, and joint space narrowing. "
            "Findings consistent with Grade II–III sacroiliitis.", height=150)

if "Spine (DISH)" in regions:
    st.header("Spinal Findings (DISH)")
    dish_level = st.radio("Confidence level (DISH):", ["Moderate", "High"])
    if dish_level == "Moderate":
        st.text_area("AI-style phrasing:", 
            ...
            "at T12–L2, with preserved disc spaces. Findings may be consistent with early DISH.", height=150)
    else:
        st.text_area("AI-style phrasing:", 
            ...
            "skeletal hyperostosis (DISH).", height=150)
