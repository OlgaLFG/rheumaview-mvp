...
try:
    uploaded_images = st.file_uploader("Upload radiographs (max 10 files)", type=["jpg", "jpeg", "png", "webp", "bmp", "tiff", "dcm"], accept_multiple_files=True)
    if uploaded_images is not None and len(uploaded_images) > 10:
        st.warning("Too many files uploaded. Please limit to 10.")
        uploaded_images = uploaded_images[:10]
except Exception as e:
    st.error("⚠️ Failed to process uploaded files.")
...
if compare == "Yes":
    try:
        ...
        st.error("⚠️ Failed to process prior imaging files.")
...
if st.button("Generate Report"):
    ...
    elif not uploaded_images or len(uploaded_images) == 0:
        st.warning("Please upload at least one current imaging file.")
    else:
        try:
            doc = Document()
            ...
            if compare == "Yes" and prior_date:
                doc.add_paragraph(f"Compared to prior imaging dated: {prior_date}")
            doc.add_paragraph("---")
            ...
            filename = "/mnt/data/rheumaview_structured_report_safe.docx"
            doc.save(filename)
